...
function getArmorAvailability1 (armors1){
    let armorsAvail = [];
...
function getTotalDefense(character){
    ...
    for (let i=0; i<character.armorUsed.length; i++){
        let addedDefense = getArmorAvailability1(character.armorUsed[i])
        total += addedDefense;
    }
    console.log(total);
}
...
function getArmorAvailability3 (armors){
    for (let i=0; i<armorsInArmory.length; i++){
        if (armors === armorsInArmory[i]){
            return armors;
        }
    }
    console.log(`"${armors}": false`);
...
function getArmorAvailability2 (armorsname){
    const armorArmor = [
        ['Dragon Scale', 'Heavy', 50],
        ['Elven Leather', 'Light', 30],
        ['Wizard Robe', 'Cloth', 15],
    ...
    let armorsAvail = [];
    for (let i=0; i<armorArmor.length; i++){
        if (armorsname === armorArmor[i][0]){
            armorsAvail.push(armorArmor[i][0]);
            console.log(`"${armorsAvail}": ${true}`);
            return armorArmor[i][2];
        }
    }
    console.log(`"${armorsname}": ${false}`);
    return 0;
}
...
function getTotalDefense(character){
    ...
    for (let i=0; i<character.armorUsed.length; i++){
        ...
        console.log(addedDefense); 
        total += addedDefense;
    }
    console.log(`Character ${character.name} with class ${character.classType} has a total defense of ${total}`);
}

function getDefenseSummary (inputCharacters){
    for (let i=0; i<inputCharacters.length; i++){
        for (let j=0; j< inputCharacters[i].armorToEquip.length; j++){
            let arUsed = getArmorAvailability3 (inputCharacters[i].armorToEquip[j])
...
getDefenseSummary(inputCharacters);